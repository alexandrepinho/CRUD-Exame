...
public class ExameAction extends ActionSupport {
	...
	public int getMedicoSelecionado() {
		return medicoSelecionado;
	}

	public void setMedicoSelecionado(int medicoSelecionado) {
	...
	public String adicionaOuAltera() {
		...
		try {
			...
			if (apto == 1) {
				exame.setResultado(true);
			} else if (apto == 0) {
				exame.setResultado(false);
			} else {
	...
	public String preparaAlteracao() {
		try {
			...
			if (exame.isResultado() == null) {
				apto = -1;
			} else {
				apto = exame.isResultado() ? 1 : 0;
			}
	...
	public String preparaCadastro() {
		try {
			...
			listaMedicos();
			listaTiposExame();
			this.exame.setFinalidade(Enum.valueOf(Finalidade.class, "ADMISSIONAL"));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
